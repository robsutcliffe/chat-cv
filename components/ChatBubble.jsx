...
export default function ChatBubble({ text, author, checkScrollPosition }) {
  ...
  const textElement = isWaiting ? (
    <Writing />
  ) : isRob ? (
    <AnimatedText baseText={text} checkScrollPosition={checkScrollPosition} />
  ) : (
    text
  );

  return (
    <div className={`flex flex-row w-full ${extraWrapperStyles}`}>
      <motion.div
      ...
      >
        <Gradient
          theme={isRob ? "primary" : "secondary"}
          style={{ filter }}
          className={`w-full h-full absolute ${extraBubbleStyle}`}
        />
        <motion.div
          animate={controls}
          ...
          style={{ background }}
          className={`shadow relative z-10 px-6 py-4 text-base md:text-lg font-ibm ${extraBubbleBackgroundStyle}`}
        >
          {textElement}
        </motion.div>
      </motion.div>
